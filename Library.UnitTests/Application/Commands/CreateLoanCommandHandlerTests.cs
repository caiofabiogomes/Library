...
namespace Library.UnitTests.Application.Commands
{
    public class CreateLoanCommandHandlerTests
    {
        ...
        public async Task InputIsOk_Executed_ReturnSucess() 
        {
            ...
            var createLoanCommand = new CreateLoanCommand(1,1,DateTime.Now.AddDays(15));
        ...
        [Fact]
        public async Task InputUserIsAdmin_Executed_ReturnFailure()
        {
            //Arrange
            var loanRepository = new Mock<ILoanRepository>();
            var userRepository = new Mock<IUserRepository>();
            ...
            var book = new Book("Livro teste", "teste", "9780134494272", new DateTime(1990, 1, 1));
            var user = new User("Usuário teste", "teste@gmail.com", "12345678", EUserRole.Admin);
            ...
            // Assert
            Assert.False(result.IsSuccess);
            Assert.Equal("não é possível realizar um empréstimo para um usuário Administrador", result.Message);
